...
interface ProtoAllGenerator {
  ...
  abstract class Factory {
    ...
    static ProtoAllGenerator create(Class annoType, MavenProject project, File protoc, File protoOut, Log log) {
      def maven = MavenHelper.Factory.create(project, log)
      def protocToPath = protoc?.isFile() ? protoc.toPath() : null

      def sourceRoot = new SourceRootImpl(maven, annoType, protocToPath, protoOut)
